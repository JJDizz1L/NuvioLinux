...
class AppUpdateAssetSelectionTest {
    ...
    fun `selects current architecture installer before other installer assets`() {
        val selected = selectBestUpdateAsset(
            assets = listOf(
                asset("NuvioLinux-0.2.0-windows-arm64.msi"),
                asset("NuvioLinux-0.2.0-windows-x64.msi"),
                asset("NuvioLinux-0.2.0.exe"),
            ),
        ...
        assertEquals("NuvioLinux-0.2.0-windows-x64.msi", selected?.name)
    }
    ...
    fun `ignores unrelated octet stream assets when desktop selector has no content types`() {
        val selected = selectBestUpdateAsset(
            assets = listOf(
                ...
                asset("NuvioLinux-0.2.0-windows-x64.zip", contentType = "application/octet-stream"),
                asset("NuvioLinux-0.2.0-windows-x64.msi", contentType = "application/octet-stream"),
            ),
            selector = AppUpdateAssetSelector(
                fileExtensions = listOf(".msi", ".exe"),
    ...
    fun `falls back to universal installer when architecture is not present`() {
        val selected = selectBestUpdateAsset(
            assets = listOf(
                asset("NuvioLinux-0.2.0-universal.msi"),
                asset("NuvioLinux-0.2.0-arm64.msi"),
            ),
            selector = AppUpdateAssetSelector(
        ...
        assertEquals("NuvioLinux-0.2.0-universal.msi", selected?.name)
    }
    ...
    fun `supports android apk content type when extension is missing`() {
        val selected = selectBestUpdateAsset(
            assets = listOf(
                asset("NuvioLinux-arm64", contentType = "application/vnd.android.package-archive"),
                asset("NuvioLinux-x86", contentType = "application/vnd.android.package-archive"),
            ),
            selector = AppUpdateAssetSelector(
        ...
        assertEquals("NuvioLinux-x86", selected?.name)
    }
    ...
    fun `returns null when no asset matches the selector`() {
        val selected = selectBestUpdateAsset(
            assets = listOf(
                asset("NuvioLinux-0.2.0-windows-x64.zip"),
                asset("checksums.txt"),
            ),
