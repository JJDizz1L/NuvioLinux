...
internal object DiscordPresenceManager {
    ...
    private fun computePresence(
        ...
        if (browse != null && config.showWhenBrowsing) {
            if (config.hideTitle) {
                return ComputedPresence(
                    activity = buildActivity(details = "Browsing Nuvio Linux", state = null, largeImage = null, largeText = null, start = null, end = null),
                    key = "browse:hide",
                    startTs = null,
                )
            }
            return ComputedPresence(
                activity = buildActivity(
                    details = browse.details ?: "Browsing Nuvio Linux",
                    state = browse.state,
    ...
    private fun buildActivity(
        ...
        val activity = buildJsonObject {
            ...
            put("details", JsonPrimitive(clean(details) ?: "Watching Nuvio Linux"))
